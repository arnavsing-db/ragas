...
CONTEXT_PRECISION_NO_ANSWER = Prompt(
    name="context_precision_no_answer",
    instruction="""Given question and context verify if the context is relevant. Give verdict as "1" if useful and "0" if not with json output. """,
    # instruction="""Given question, answer and context verify if the context was useful in arriving at the given answer. Give verdict as "1" if useful and "0" if not with json output. """,
    # examples=[
    #     {
    #         "question": """What can you tell me about albert Albert Einstein?""",
...
class ContextPrecision(MetricWithLLM):
    ...
    def _calculate_average_precision(self, json_responses: t.List[t.Dict]) -> float:
        ...
        verdict_list = [
            ...
            if resp.get("verdict") is not None
            else np.nan
...
@dataclass
class ContextPrecisionNoAnswer(MetricWithLLM):
    """
    Average Precision is a metric that evaluates whether all of the
    relevant items selected by the model are ranked higher or not.
    ...
    async def _ascore(
        ...
        human_prompts = self._context_precision_prompt_no_answer(row)
        responses: t.List[str] = []
        for hp in human_prompts:
            result = await self.llm.generate(
                hp,
...
context_utilization_no_answer = ContextUtilizationNoAnswer()
